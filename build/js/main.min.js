...
window.addEventListener('DOMContentLoaded', function () {
  ...
  updateInitialPosition();
  var navigationButton = document.querySelector('.navigation__wrapper button');
  ...
  navigationButton.addEventListener('click', function () {
    ...
    if (navigationList.classList.contains('active')) {
      var navListHeight = navigationList.scrollHeight;
      navigationList.style.height = "".concat(navListHeight, "px");
    } else {
  ...
  var wrapperTextElements = document.querySelectorAll('.wrapper-text-content');
  var contentContainres = document.querySelectorAll('.content-container');
  if (contentContainres.length > 1) {
    for (var i = 0; i < contentContainres.length - 1; i++) {
      contentContainres[i].classList.add('content-container-border');
    }
  } else {
    if (wrapperTextElements.length > 0) {
      for (var _i = 0; _i < wrapperTextElements.length - 1; _i++) {
        wrapperTextElements[_i].classList.add('wrapper-text-border');
      }
    }
  }
  function processRows(sw) {
    var slides = Array.from(sw.querySelectorAll('.swiper-slide'));
    var maxRows = Math.max.apply(Math, _toConsumableArray(slides.map(function (slide) {
      return slide.querySelectorAll('.row').length;
    })));
    var rowElements = [];
    var h = 0;
    sw.querySelectorAll('.row').forEach(function (i) {
      return i.style.height = 'auto';
    });
    for (var _i2 = 0; _i2 < maxRows; _i2++) {
      rowElements[_i2] = Array.from(sw.querySelectorAll(".swiper-slide .row:nth-child(".concat(_i2 + 1, ")")));
    }
    for (var j = 0; j < rowElements.length; j++) {
      h = 0;
      for (var _i3 = 0; _i3 < rowElements[j].length; _i3++) {
        if (rowElements[j][_i3].offsetHeight > h) {
          h = rowElements[j][_i3].offsetHeight;
        }
      }
      for (var _i4 = 0; _i4 < slides.length; _i4++) {
        rowElements[j][_i4].style.height = "".concat(h, "px");
      }
    }
  }
  function assignUniqueClassToChildren() {
    var _this = this;
    var tableSwipers = document.querySelectorAll('.table-swiper');
    var uniqueClassIndex = 0;
    tableSwipers.forEach(function (tableSwiper) {
      var child = tableSwiper.children[0];
      var uniqueClass = "table-swiper-".concat(uniqueClassIndex);
      child.classList.add(uniqueClass);
      var slides = document.querySelector(".".concat(uniqueClass)).querySelectorAll('.swiper-slide');
      var slidesCount = slides.length;
      if (slidesCount <= 4 && window.innerWidth >= 768) {
        var slideWidth = (100 / slidesCount).toFixed(2) + '%';
        slides.forEach(function (slide) {
          slide.style.maxWidth = slideWidth;
        });
        document.querySelector(".".concat(uniqueClass)).style.paddingBottom = '0px';
      }
      if (window.innerWidth < 768) {
        slides.forEach(function (slide) {
      ...
      if (slidesCount > 4 && window.innerWidth >= 768) {
        ...
        document.querySelector(".".concat(uniqueClass)).style.paddingBottom = '40px';
        document.querySelector(".".concat(uniqueClass)).querySelector('.swiper-pagination').style.setProperty('bottom', '0', 'important');
      }
      var swiper = new Swiper(".".concat(uniqueClass), {
        pagination: {
          el: '.swiper-pagination',
          clickable: true
        },
      ...
      processRows(_this.document.querySelector(".".concat(uniqueClass)));
      window.addEventListener('resize', function () {
        processRows(this.document.querySelector(".".concat(uniqueClass)));
      });
      uniqueClassIndex++;
    });
  }
  function adjustH3Heights() {
    var wrapperProducts = document.querySelector('.wrapper-products');
    if (!wrapperProducts) return;
    var h3Elements = wrapperProducts.querySelectorAll('h3');
    var maxHeight = 0;
    h3Elements.forEach(function (h3) {
      h3.style.height = 'auto';
      var height = h3.offsetHeight;
      maxHeight = Math.max(maxHeight, height);
    });
    h3Elements.forEach(function (h3) {
      h3.style.height = "".concat(maxHeight, "px");
